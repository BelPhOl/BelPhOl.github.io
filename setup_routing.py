...
DEPRECATED (2026-08-12): sitemap.xml and robots.txt are no longer written by
this script. Ownership moved to the belpho-platform generator's
lib/sitemap.js (run via `node build.js --site`), which fixes the stale
`belpho.by` host to `belpho.org`, turns sitemap.xml into a sitemap index,
and reconstructs the same 394 alumni URLs this script used to emit —
byte-compatible, read straight from data/alumni.json each build so it can
never drift from this file's own sanitize_filename() logic. Do not call
create_sitemap()/create_robots_txt() again; running setup_all_routing() will
now skip them (see below) rather than silently re-introduce the stale host.
create_htaccess()/create_nginx_config()/create_url_map() are unaffected and
still run — GitHub Pages ignores .htaccess/nginx.conf either way, but
nothing here currently reads or fixes that, so it is left as-is.
"""
...
"""
    ...
    def create_sitemap(self):
        """DEPRECATED — do not call. sitemap.xml is now owned by
        belpho-platform's lib/sitemap.js (`node build.js --site`), which
        also fixes the belpho.by host this method still hard-codes below.
        Left in place for reference only; setup_all_routing() no longer
        calls it."""
        raise RuntimeError(
            "create_sitemap() is retired: sitemap.xml is now generated by "
            "belpho-platform (node build.js --site), not setup_routing.py. "
            "Run that instead."
        )
...
"""
    ...
    def create_robots_txt(self):
        """DEPRECATED — do not call. robots.txt is now owned by
        belpho-platform's lib/sitemap.js (`node build.js --site`), which
        also fixes the belpho.by host this method still hard-codes below.
        Left in place for reference only; setup_all_routing() no longer
        calls it."""
        raise RuntimeError(
            "create_robots_txt() is retired: robots.txt is now generated by "
            "belpho-platform (node build.js --site), not setup_routing.py. "
            "Run that instead."
        )
...
"""
    ...
    def setup_all_routing(self):
        ...
        self.create_url_map()
        # create_sitemap() / create_robots_txt() retired 2026-08-12 — see
        # module docstring. sitemap.xml/robots.txt now come from
        # belpho-platform's `node build.js --site`.
